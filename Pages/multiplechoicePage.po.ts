...
export class tasksPage {
    ...
    resourcePanelElement() {
        return element(by.xpath("//div[contains(text(),'Resources')]"));
    }
    createNewTaskBtn() {
    ...
    Picture3() {
        return element(by.xpath("(//div[text()=' Picture '])[3]"));
    }
    Picture4() {
        return element(by.xpath("(//div[text()=' Picture '])[4]"));
    }
        ...
        if (dataRecs && dataRecs.length > 0) {
            dataRecs.forEach(record => {
                ...
                this.saveMCQ().isDisplayed().then(function (dis) {
                    expect(dis).toBe(true, ' MCQwithtexttotext task is created successfully')
                })
            })
        }
    }
    MCQTexttoPicturetelugu() {
        var dataRecs = dataProvider.getJsonDataFromFile('./TestData/MCQTaskData/TeluguData.json', null)
        if (dataRecs && dataRecs.length > 0) {
            dataRecs.forEach(record => {
                browser.ignoreSynchronization = true
    ...
    MCQPicturetoPicturetelugu() {
        ...
        if (dataRecs && dataRecs.length > 0) {
            dataRecs.forEach(record => {
                ...
                this.typefile10().sendKeys(DiesPath);
                browser.sleep(2000);
                ...
                this.typefile14().sendKeys(finger4Path);
                browser.sleep(2000);
                var finger5 = "../../TestData/ImageFiles/finger5.png";
                var finger5Path = path.resolve(__dirname, finger5);
                this.typefile15().sendKeys(finger5Path);
                browser.sleep(2000);
                this.checkbox3().click();
                this.saveMCQ().isDisplayed().then(function (dis) {
                    expect(dis).toBe(true, ' MCQPicturetoPicture task is created successfully')
                })
            })
        }
    }
    MCQPicturetoPictureenglish() {
        var dataRecs = dataProvider.getJsonDataFromFile('./TestData/MCQTaskData/EnglishData.json', null)
        if (dataRecs && dataRecs.length > 0) {
            dataRecs.forEach(record => {
                browser.ignoreSynchronization = true
    ...
    MCQPicturetoTextEnglish() {
        ...
        if (dataRecs && dataRecs.length > 0) {
            dataRecs.forEach(record => {
                ...
                browser.sleep(1000);
                this.saveMCQ().isDisplayed().then(function (dis) {
                    expect(dis).toBe(true, ' MCQPicturetoText task is created successfully')
                })
            })
        }
    }
    createTappingMCQTasktexttoimageenglish() {
        this.mcqtaskcreatepageenglish();
        this.MCQPicturetoTextEnglish();
        browser.sleep(2000);
        this.saveMCQ().click();
        browser.sleep(10000);
        this.okbutton().click();
    ...
    UpdateMCQ() {
        ...
        if (dataRecs && dataRecs.length > 0) {
            dataRecs.forEach(record => {
                ...
                browser.sleep(2000);
                var linkselection = by.xpath("(//td[contains(text(),'Draft')]/..//td/span//a[contains(text(),'Multiplechoice')])[1]");
                ...
                this.Publishnavigation();
                this.createNewTaskBtn().isDisplayed().then(function (dis) {
                    expect(dis).toBe(true, 'MCQ is updated successfully')
                })
            })
        }
    }
    ...
    Previewpageformultiplequestions() {
        browser.sleep(4000);
        this.Previewradio1().click();
        browser.sleep(2500);
        this.checkbtn().click();
